...
Function Decode-Files($hash) {
  foreach ($key in $hash.keys) {
    ...
    New-Object psobject -Property @{ dst = $dst; verifies = $sMd5 -like $dMd5; src_md5 = $sMd5; dst_md5 = $dMd5; tmpfile = $tmp; tmpzip = $tzip }
  }
}
...
Function Unzip-File($src, $dst) {
  ...
  if (-not (test-path $unpack)) {$null = mkdir $unpack }
